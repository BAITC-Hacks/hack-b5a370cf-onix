...
**Живая версия:** https://hack-b5a370cf-onix.vercel.app (Vercel; симулятор, карта, оптимум и стресс-тест работают всегда, AI-советник и общий рейтинг включаются при заданных на Vercel переменных LLM и Upstash — см. `.env.example`).
...
Веб-симулятор, в котором пользователь распределяет единый бюджет (100 у.е.) Астаны на **ровно 5 управленческих решений** по транспорту, экологии, соцсфере, безопасности и городским сервисам. После проверки полного плана он видит **Astana Quality of Life Score**, изменения показателей каждого района и **AI-объяснение**: сильные стороны, риски, компромиссы и конкретные рекомендации.

Главный принцип: **числа считает детерминированный движок, AI только объясняет**. LLM получает готовый результат расчёта. Числовые утверждения автоматически сравниваются с фактами движка; ответ с неподтверждёнными числами или неверной трактовкой синергии заменяется расчётным шаблоном.
...
| `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` | нет локально; обе нужны для Vercel | — | Постоянный общий рейтинг команд через Upstash Redis REST. Токен только на сервере; не используйте `NEXT_PUBLIC_` |

Если LLM недоступен (нет ключа, ошибка сети, таймаут 25 с, битый JSON) или его ответ не проходит проверку фактов, пользователь получает шаблонный анализ и видит пометку о причине. Приложение не падает. Публичные AI-маршруты ограничивают размер запроса и число обращений; на Vercel для общих лимитов требуется Upstash Redis.

## Как проверить основной сценарий (≈2 минуты)

1. **Автотесты движка:** `npm test`. Должны пройти проверки контрольных чисел (база 52.56, пример из ТЗ 56.54), валидатора, оптимизатора и защиты AI-ответов.
...
13. **Рейтинг команд:** отключите городское событие, затем на странице «План» нажмите «🏆 Отправить в рейтинг», введите название команды и отправьте. Сервер пересчитает Score и худший случай и покажет место. Все команды соревнуются на одной базовой ситуации; обновить запись может только тот же браузер. Откройте `/leaderboard` с другого устройства в той же сети — таблица общая.
...
| `lib/leaderboard.ts`, `lib/upstash.ts`, `app/api/leaderboard/route.ts`, `components/pages/LeaderboardPage.tsx` | Общий рейтинг: Upstash Redis REST при настройке обеих переменных, локальный файл без них; атомарная запись, одна команда — одна строка, топ-500 |
...
**Технологии:** Next.js 16 (App Router; production-сборка через webpack), React 19, TypeScript, Tailwind CSS 4, Node test runner. LLM вызывается через REST API напрямую `fetch`-ом, без SDK. Runtime-зависимости: next, react, react-dom, maplibre-gl.
...
| Описывает компромиссы: какие направления и районы остались без вложений | Числа в ответе сверяются с фактами; для Score, среднего балла и баллов районов проверяется также тип метрики. При несовпадении или неверной трактовке синергии текст модели заменяется проверенным шаблоном |
...
| Опционально | Статус |
|---|---|
| Сравнение результатов нескольких команд или сценариев | ✅ **Общий рейтинг команд** (`/leaderboard`): принимает только планы без события, чтобы начальные условия были одинаковыми; Score и худший случай по кризисам считает сервер, запись команды защищена токеном браузера. Плюс локальное сравнение сценариев «В сравнение» на «Результатах» |
...
- Локальные сохранённые сценарии и токен владения записью команды хранятся в `localStorage` браузера. Общий рейтинг без Upstash сохраняется в `data/leaderboard.json` и подходит для локального запуска; на Vercel для постоянного рейтинга и ограничений запросов задайте обе `UPSTASH_REDIS_REST_*` переменные. При ошибке подключения рейтинг и платные AI-маршруты временно недоступны.
- Качество формулировок AI зависит от модели. Движок гарантирует расчёт Score и показателей; ответ модели с неподтверждёнными числами или обнаруженным противоречием не показывается, вместо него выводится проверенный расчётный текст. Защита от любых смысловых ошибок естественного языка не гарантируется.
...
- Развитие: цепочки событий по кварталам, многоэтапное планирование по кварталам, реальные данные акимата вместо синтетики.
