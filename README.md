...
Главный принцип: **числа считает детерминированный движок, AI только объясняет**. LLM получает готовый результат расчёта. Числовые утверждения автоматически сравниваются с фактами движка; неподтверждённые числа и противоречия в описании синергий явно помечаются.
...
**Новый путь через конкретное городское решение:** откройте `/plan`. В верхней части страницы показан условный участок Нуры: базовый показатель школ и детсадов S1 равен 38, озеленения E1 — 45. Выберите школу с детсадом (M7) или парк (M4). Выбранный проект появится на обеих картах — реальной и схематичной; на `/results` дорожная карта покажет строительство и начало эффекта по кварталам. Затем соберите остальные четыре меры из каталога и сравните последствия. Геометрия объекта иллюстративна: данных о конкретных земельных участках и доступности квартала в датасете нет. Стоимость, лаг и изменения районных показателей по-прежнему считаются по ТЗ, без дополнительной географической поправки.

Например, при одинаковых остальных мерах M8/Нура + M10/Нура + M12 + M5/Сарыарка школа в Нуре даёт Score 56.54 за 95 у.е. и выводит S1 из критической зоны (38 → 48). Парк стоит дешевле: план обходится в 86 у.е. и повышает E1 с 45 до 54, но S1 остаётся 38, итоговый Score 55.58. Так выбор на карте выражает реальный компромисс, уже заложенный в правилах M4+M7.
...
| Описывает компромиссы: какие направления и районы остались без вложений | Числа в ответе сверяются с фактами; для Score, среднего балла и баллов районов проверяется также тип метрики. Несовпадения и неверная трактовка синергий показываются предупреждением |
...
- Качество формулировок AI зависит от модели. Движок гарантирует расчёт Score и показателей; проверка подсвечивает неподтверждённые числа и часть неверных смысловых привязок в тексте LLM.
