...
1. **Автотесты движка:** `npm test`. Должно быть 11 из 11 passed: база 52.56, пример из ТЗ 56.54, валидатор, оптимизатор.
...
9. **AI-советник** (нужен `OPENAI_API_KEY`): нажмите подсказку «Найди лучший план без ЛРТ и в бюджете 80». Агент сам вызовет перебор планов с ограничениями (без M3, бюджет ≤ 80), покажет шаги и предложит план (Score 56.87, стоимость 72). Кнопка «Применить» загружает его в симулятор.
10. **Отчёт:** в панели плана нажмите «Отчёт / PDF». Откроется одностраничный отчёт-презентация решения с графиками и AI-разбором, кнопка «Сохранить PDF» печатает его.
...
12. **Стресс-тест:** для оптимального плана блок «Стресс-тест плана» покажет «провал в 2 из 6 сценариев». «Найти устойчивый план» предложит план с худшим случаем 55.69, выполнимый во всех событиях. Советник отвечает на «Выдержит ли мой план кризисы?» инструментами stress_test_plan и find_robust_plan.
13. **Городское событие:** выберите «Сокращение трансферта». Бюджет станет 85, набор из шага 3 (стоимость 95) станет невалидным, валидатор покажет причину. Выберите «Прорыв теплотрассы в Алматы»: ЖКХ Алматы упадёт до 38 (ниже 40, штраф), и AI-анализ предложит заменить M5 на M14 (+1.44).
...
| `lib/agent.ts` | AI-советник: OpenAI tool calling поверх движка (simulate_plan, find_best_plans, stress_test_plan, find_robust_plan, propose_plan), до 6 шагов, проверка чисел в ответе |
...
| Визуализация изменений показателей районов | ✅ **Реальная 3D-карта Астаны** (MapLibre GL): настоящие границы 5 районов из OpenStreetMap (скачаны в `public/data/astana-districts.geojson`, работают без сети), подложка OpenFreeMap; высота и цвет района равны баллу или выбранному показателю, анимация при изменении плана. Запасной режим «Схема»:  (изометрия, реальное взаимное расположение районов и р. Есиль): высота района равна баллу или выбранному показателю, анимация при изменении плана, пунктир «до мер», метки показателей < 40. Плюс графики «до/после», вклад мер и тепловая таблица |
...
| Агентный AI | ✅ AI-советник: цикл tool calling (simulate_plan, find_best_plans с ограничениями, propose_plan); каждый шаг агента виден пользователю, предложенный план проходит валидатор |
...
- AI-советник работает только с LLM (OpenAI-совместимый API). Без ключа он честно сообщает об этом, остальные функции работают.
