...
1. **Автотесты движка:** `npm test`. Должно быть 14 из 14 passed: база 52.56, пример из ТЗ 56.54, валидатор, оптимизатор.
...
8. На странице «План» нажмите «В сравнение»: сценарий появится в таблице сравнения на «Результатах» (сортировка по Score).
...
10. **Отчёт:** в панели плана нажмите «Отчёт / PDF». Откроется одностраничный отчёт-презентация решения с графиками и AI-разбором, кнопка «Сохранить PDF / Печать» печатает его.
...
13. **Городское событие:** выберите «Сокращение трансферта из республиканского бюджета». Бюджет станет 85, набор из шага 3 (стоимость 95) станет невалидным, валидатор покажет причину. Выберите «Прорыв теплотрассы в Алматы»: ЖКХ Алматы упадёт до 38 (ниже 40, штраф), и AI-анализ предложит заменить M5 на M14 (+1.44).
...
| Опционально | Статус |
|---|---|
| Сравнение результатов нескольких команд или сценариев | ✅ «В сравнение», таблица с сортировкой по Score |
