...
import { districtName, type Decision } from "@/lib/engine";
  ...
  const ask = async (text: string) => {
    ...
    try {
      const res = await fetch("/api/agent", {
        ...
        body: JSON.stringify({ messages: next.map(({ role, content }) => ({ role, content })), decisions, eventId }),
      });
        ...
        <div className="max-h-[560px] space-y-4 overflow-y-auto pr-1">
          {items.map((it, i) =>
            it.role === "user" ? (
              ...
              <div key={i} className="max-w-[92%] space-y-2">
                {it.reply?.steps?.length ? (
                  <ol className="space-y-1 border-l-2 border-line pl-3 text-xs text-ink-2">
                    {it.reply.steps.map((s, j) => (
                      <li key={j}>
                        <span className="font-semibold text-ink">⚙ {s.tool}:</span> {s.summary}
                      </li>
                    ))}
                  </ol>
                ) : null}
                ...
                <div className="flex flex-wrap gap-2">
                  {it.reply?.model && <StatusBadge kind="info">агент · {it.reply.model}</StatusBadge>}
                  {it.reply?.unverifiedNumbers && it.reply.unverifiedNumbers.length === 0 && <StatusBadge kind="good">Числа сверены с движком</StatusBadge>}
                  {it.reply?.unverifiedNumbers && it.reply.unverifiedNumbers.length > 0 && (
                    <StatusBadge kind="crit">Не из расчёта: {it.reply.unverifiedNumbers.join(", ")}</StatusBadge>
                  )}
          ...
          {loading && (
            <div className="flex items-center gap-2 text-sm text-ink-3">
              <span className="inline-block size-2 animate-pulse rounded-full bg-accent" /> Агент проверяет варианты на движке…
            </div>
          )}
          <div ref={endRef} />
        </div>
